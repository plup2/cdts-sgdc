//TODO: language split, theme split, update soy compiler, linting? linting soy?, allyt testing, generation of static fallback files?, actual build pipeline.
...
module.exports = function(grunt) {
    ...
    function cdtsContentReplace(content, srcpath) {
        ...
        //Remove bulky DEBUG statements generated by new SOY compiler in "wet-*.js" files
        if (srcpath.endsWith('/wet-en.js') || srcpath.endsWith('/wet-fr.js')) {
            //NOTE: The following regex catches the generated debug source code that can be line-broken at various places
            //      That should leave us with 2 "debugSoyTemplateInfo" strings in the file. We issue a warning if it is not the case.
            vtr = vtr.replace(/\+[\n\r]*?\(goog\.DEBUG\&\&[\n\r]*?soy\.\$\$debugSoyTemplateInfo\?.*?\)/gs, '');
            
            if ((vtr.match(/debugSoyTemplateInfo/gs) || []).length !== 2) {
                console.warn('***WARNING*** Possible discrepency detected. Was the SOY compiler updated recently? You may want to review regular expression that removes the goog.DEBUG entries in Grunfile.js#cdtsContentReplace');
            }
        }
    ...
    require('google-closure-compiler').grunt(grunt, { //https://repo1.maven.org/maven2/com/google/javascript/closure-compiler/ https://github.com/google/closure-compiler-npm/blob/master/packages/google-closure-compiler/docs/grunt.md (https://www.npmjs.com/package/google-closure-compiler)
    ...
    grunt.registerTask('build', 'Run non-minified build', ['clean', 'copy-public', 'unzip:soyutils', 'build-soy', 'genstatic']);
    grunt.registerTask('copy-public', 'Copy all public files', ['copy:wet', 'copy:gcweb-public', 'copy:gcintranet-public']);
    ...
    grunt.registerTask('build-soy', 'Produce Javascript from soy templates', ['compile-soy', 'closure-compiler', 'concat']);
    ...
    grunt.initConfig({
        ...
        unzip: {
            soyutils: {
                src: ['./compilers/jssrc_js.jar'],
                dest: '<%= project.temp %>/goog_soyutils',
            },
        },
        ...
        run_java: {
            ...
            'gcweb': {
                ...
                javaArgs: '--outputPathFormat <%= project.temp %>/gcweb/wet-{LOCALE}-soy.js --messageFilePathFormat ./src/gcweb/wet-messages.{LOCALE}.xlf --locales en,fr --srcs ./src/gcweb/appPage.soy,./src/gcweb/wet.soy,./src/gcweb/serverPage.soy',
            },
            'gcintranet': {
                command: 'java',
                jarName: 'compilers/SoyToJsSrcCompiler.jar',
                javaArgs: '--outputPathFormat <%= project.temp %>/gcintranet/wet-{LOCALE}-soy.js --messageFilePathFormat ./src/gcintranet/wet-messages.{LOCALE}.xlf --locales en,fr --srcs ./src/gcintranet/wet.soy,./src/gcintranet/serverPage.soy',
            },
        },
        exec: {
        ...
        'closure-compiler': {
            'gcweb-en': {
                files: {
                    '<%= project.temp %>/gcweb/wet-en.js': ['<%= project.temp %>/gcweb/wet-en-soy.js']
                },
                options: {
                    js: ['./node_modules/google-closure-library', './tmp/goog_soyutils/soyutils_usegoog.js', './tmp/goog_soyutils/checks.js'],
                    entry_point: '<%= project.temp %>/gcweb/wet-en-soy.js',
            ...
            'gcweb-fr': {
                files: {
                    '<%= project.temp %>/gcweb/wet-fr.js': ['<%= project.temp %>/gcweb/wet-fr-soy.js']
                },
                options: {
                    js: ['./node_modules/google-closure-library', './tmp/goog_soyutils/soyutils_usegoog.js', './tmp/goog_soyutils/checks.js'],
                    entry_point: '<%= project.temp %>/gcweb/wet-fr-soy.js',
                    compilation_level: 'SIMPLE',
                    dependency_mode: 'PRUNE',
                },
            },
            'gcintranet-en': {
                files: {
                    '<%= project.temp %>/gcintranet/wet-en.js': ['<%= project.temp %>/gcintranet/wet-en-soy.js']
                },
                options: {
                    js: ['./node_modules/google-closure-library', './tmp/goog_soyutils/soyutils_usegoog.js', './tmp/goog_soyutils/checks.js'],
                    entry_point: '<%= project.temp %>/gcintranet/wet-en-soy.js',
                    compilation_level: 'SIMPLE',
                    dependency_mode: 'PRUNE',
                },
            },
            'gcintranet-fr': {
                files: {
                    '<%= project.temp %>/gcintranet/wet-fr.js': ['<%= project.temp %>/gcintranet/wet-fr-soy.js']
                },
                options: {
                    js: ['./node_modules/google-closure-library', './tmp/goog_soyutils/soyutils_usegoog.js', './tmp/goog_soyutils/checks.js'],
                    entry_point: '<%= project.temp %>/gcintranet/wet-fr-soy.js',
                    compilation_level: 'SIMPLE',
                    dependency_mode: 'PRUNE',
                },
            },
